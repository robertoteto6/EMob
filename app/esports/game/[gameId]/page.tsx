...
async function fetchGameData(gameId: string) {
  const [matchesRes, tournamentsRes, teamsRes, playersRes] = await Promise.allSettled([
    fetch(`/api/esports/matches?game=${gameId}`, { cache: "no-store" }),
    fetch(`/api/esports/tournaments?game=${gameId}`, { cache: "no-store" }),
    fetch(`/api/esports/teams?game=${gameId}`, { cache: "no-store" }),
    fetch(`/api/esports/players?game=${gameId}`, { cache: "no-store" })
  ]);
