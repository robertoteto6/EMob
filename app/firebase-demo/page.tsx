...
type FirebaseTab = 'auth' | 'profile' | 'predictions';

const FirebaseDemoPage: React.FC = () => {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<FirebaseTab>('auth');
  ...
  const TabButton: React.FC<{ tab: FirebaseTab; label: string; isActive: boolean }> = ({ tab, label, isActive }) => (
    <button
